...
(function () {
  ...
  function parseQuery(queryStr) {
    ...
    if (varMatch) {
      ...
      for (var i = 0; i < parts.length; i++) {
        var pieces = parts[i].split(":").map(function (s) {
          return s.trim();
        });
        if (pieces.length >= 2) {
  ...
  window.loadPastQueries = function () {
    ...
    var graphqlEntries = entries.filter(function (e) {
      return e.name.includes("/graphql");
    });
    console.log(
      "Found " + graphqlEntries.length + " past GraphQL requests in performance timeline.",
    );
    console.log(
      "(These only show URLs, not bodies. New requests will be captured with full query text.)",
    );
    return graphqlEntries.map(function (e) {
      return e.name;
    });
  };
