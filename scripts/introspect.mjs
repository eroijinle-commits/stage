...
function printSummary(schema) {
    ...
    const keyTypes = schema.types.filter(t =>
        t.kind === "OBJECT" &&
        t.fields &&
        t.name &&
    ...
    const allObjects = schema.types.filter(t =>
        t.kind === "OBJECT" && t.name && !t.name.startsWith("__")
    );
...
function browserIntrospect() {
    ...
    return fetch("https://stake.com/_api/graphql", {
        ...
        .then(function (body) {
            if (body.errors) { console.error("Introspection blocked:", body.errors[0].message); return null; }
            ...
            var keyTypes = schema.types.filter(function (t) {
                return t.kind === "OBJECT" && t.fields && t.name && !t.name.startsWith("__") &&
                    /fixture|market|sport|group|template|outcome|bet/i.test(t.name);
            });
            if (keyTypes.length > 0) {
                console.log("\n-- Key Types (" + keyTypes.length + ") --\n");
...
async function main() {
    ...
    const headers = { "Content-Type": "application/json", "x-language": "en", "x-operation-name": "IntrospectionQuery", "x-operation-type": "query" };
    if (token) headers["x-access-token"] = token;
