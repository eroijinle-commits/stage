...
import { executeQuery, setToken, testConnectionQuery } from "@/lib/stake-api/client";
...
describe("testConnectionQuery", () => {
  ...
  it("returns true when API responds with valid data", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(graphqlSuccess({ user: { balances: [{ currency: "NGN" }] } })),
    );
...
describe("Error classification", () => {
  ...
  it("classifies duplicate fixture errors", () => {
    expect(
      classifyError(new Error("Multi bet cannot contain multiple markets from same event")),
    ).toBe("duplicateFixtures");
