...
export default function SettingsPage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto p-4 gap-6 max-w-2xl">
      <section className="space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Your Stake API token for live data and bet placement.
          </p>
        </div>
        <div className="bg-card border border-border rounded p-4 space-y-3">
          ...
          <div className="flex items-center gap-2 text-xs font-mono">
            <div
              className={`w-2 h-2 rounded-full ${apiToken ? "bg-bet-won" : "bg-bet-cancelled"}`}
            />
      ...
      <section className="space-y-4">
        ...
        <div className="bg-card border border-border rounded p-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Select
            ...
            value={oddsFormat}
            onChange={(v) => {
              setOddsFormat(v as typeof oddsFormat);
              updateSettings({ oddsFormat: v as typeof oddsFormat });
            }}
          />
        </div>
      </section>
        ...
        <div className="bg-card border border-border rounded p-4 space-y-3">
          {(
            ...
            <div key={n.key} className="flex items-center justify-between">
              ...
              >
                <span
                  className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform shadow-sm ${notifications[n.key] ? "translate-x-4" : "translate-x-0.5"}`}
                />
              </button>
            </div>
          ))}
        ...
        <div className="bg-card border border-border rounded p-4 space-y-2">
          {presetsLoading ? (
            ...
            <p className="text-xs font-mono text-muted-foreground">
              No staking presets configured yet.
            </p>
          ) : (
            presets.map((p) => (
              <div
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono text-muted-foreground capitalize">
                    ...
                    {p.mode === "percentage"
                      ? `${p.percentage}%`
                      : p.mode === "kelly"
                        ? `bankroll: ₦${p.bankroll?.toLocaleString("en-NG")}`
      ...
      <section className="space-y-4">
        <h2 className="text-xs font-mono font-semibold text-foreground uppercase tracking-wider text-bet-lost">
          Danger Zone
        </h2>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] font-mono text-muted-foreground">
                This will permanently delete all local bet records.
              </p>
            </div>
