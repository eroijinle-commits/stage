...
interface ComputeSlipDetailProps {
...
export default function ComputeSlipDetail({ slip, currency, onRemove, onPlaceBets }: ComputeSlipDetailProps) {
    ...
    if (slip.selections.length === 0) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center text-xs font-mono text-muted-foreground gap-2">
                ...
                <span>This compute slip has no selections.</span>
            </div>
        );
    }
        ...
        <div className="flex flex-col h-full overflow-hidden">
            <div className="shrink-0 flex items-center justify-between px-3 py-1.5 border-b border-border bg-card/30">
                <div className="flex items-center gap-2">
                    <span className="text-xs font-mono font-semibold text-foreground">{slip.name}</span>
                    <span className="text-[10px] font-mono text-muted-foreground">({slip.selections.length} legs)</span>
                </div>
                ...
                {(["singles", "parlay"] as const).map((m) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                "px-2 py-0.5 text-[11px] font-mono rounded transition-colors capitalize",
                                slip.mode === m
                ...
                <table className="w-full text-[11px] font-mono">
                    <thead className="sticky top-0 bg-card z-10 border-b border-border">
                        <tr className="text-muted-foreground">
                            <th className="text-left px-3 py-1.5 font-medium">Fixture</th>
                            <th className="text-left px-2 py-1.5 font-medium">Selection</th>
                    ...
                    <tbody className="divide-y divide-border/50">
                        {slip.selections.map((s) => {
                            const rowReturn = s.odds * slip.stakePerLeg;
                            return (
                                <tr key={s.id} className="hover:bg-muted/20 transition-colors">
                                    ...
                                    <td className="px-2 py-1.5 text-right">
                                        <span className="text-foreground tabular-nums">
                                            {currency} {rowReturn.toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </span>
                                    </td>
                                </tr>
                            );
            ...
            <div className="shrink-0 border-t border-border p-3 space-y-1.5">
                ...
                {slip.mode === "parlay" && slip.selections.length >= 3 && (
                    <button
                        ...
                        className={cn(
                            "w-full flex items-center justify-between py-1 px-2 rounded text-[11px] font-mono transition-colors",
                    ...
                    >
                        ...
                        <span className={cn(
                            ...
                            <span className={cn(
                                ...
                                slip.stakeShieldEnabled ? "translate-x-2.5" : "translate-x-0.5",
                            )} />
                        </span>
                    </button>
                )}
