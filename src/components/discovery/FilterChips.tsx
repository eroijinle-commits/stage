...
export default function FilterChips({
  ...
  if (filters.dateFrom) {
    const from = new Date(filters.dateFrom).toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "short",
    });
  ...
  if (filters.dateTo) {
    const to = new Date(filters.dateTo).toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "short",
    });
  ...
  if (filters.tournamentSlugs.length > 0) {
    chips.push({
      key: "tournamentSlugs",
      label: `${filters.tournamentSlugs.length} league${filters.tournamentSlugs.length > 1 ? "s" : ""}`,
    });
  }
