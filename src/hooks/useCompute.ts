...
import type { ComputeConfig, ComputeResult, ComputeSlip, RankedMarket } from "@/lib/compute/types";
...
export function useCompute(fixture: DiscoveryFixture | null): UseComputeReturn {
  ...
  const availableSlipCounts = useMemo(() => SLIP_OPTIONS[config.maxOutcomes], [config.maxOutcomes]);

  // ─── Reset slipCount when maxOutcomes changes ──────────────────────────

  const setConfigStable = useCallback((next: ComputeConfig) => {
    setConfig((prev) => {
