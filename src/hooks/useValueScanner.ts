...
import { getSportIndex, getFixtureDetailsQuery } from "@/lib/stake-api";
...
function analyzeMarketGap(market: StakeMarket): FlaggedMarket | null {
  const activeOutcomes = market.outcomes.filter((o: StakeMarketOutcome) => o.active && o.odds > 0);
...
export function useValueScanner(
  ...
  const fetchFixtures = useCallback(async () => {
    ...
    try {
      ...
      if (controller.signal.aborted) return;
      const ctx = buildScannerErrorContext(err, "fetch-fixtures", sport, filtersRef.current);
  ...
  const flaggedResults = useMemo(() => {
    ...
    for (const fixture of rawFixtures) {
      ...
      for (const market of markets) {
        ...
        const activeOutcomes = market.outcomes.filter((o: StakeMarketOutcome) => o.active);
  ...
  const totalFlaggedMarkets = flaggedResults.reduce((sum, r) => sum + r.flaggedMarkets.length, 0);
  ...
  useEffect(() => {
    if (
      prevPhaseRef.current === "analyzing" &&
      phase === "analyzing" &&
