...
async function introspect() {
    ...
    if (body.errors) { console.error("Introspection blocked:", body.errors[0].message); return; }
    ...
    for (var i = 0; i < schema.queryType.fields.length; i++) {
        ...
        var args = f.args.length > 0 ? "(" + f.args.map(function (a) { return a.name + ": " + fmtType(a.type); }).join(", ") + ")" : "";
    ...
    if (schema.mutationType) {
        ...
        for (var j = 0; j < schema.mutationType.fields.length; j++) {
            ...
            var margs = mf.args.length > 0 ? "(" + mf.args.map(function (a) { return a.name + ": " + fmtType(a.type); }).join(", ") + ")" : "";
            console.log("  " + mf.name + margs + " \u2192 " + fmtType(mf.type));
        }
    }

    var keyTypes = schema.types.filter(function (t) {
        return t.kind === "OBJECT" && t.fields && t.name && !t.name.startsWith("__") && /fixture|market|sport|group|template|outcome/i.test(t.name);
    });
        ...
        for (var k = 0; k < keyTypes.length; k++) {
            ...
            if (t.fields) {
                for (var m = 0; m < t.fields.length; m++) {
                    var ff = t.fields[m];
                    console.log("    " + ff.name + ": " + (ff.type.name || (ff.type.ofType && ff.type.ofType.name) || ff.type.kind));
                }
            }
            console.log("  }\n");
        }
...
export async function introspectStakeAPI(): Promise<IntrospectionResult> {
  ...
  `;
    return executeQuery<IntrospectionResult>({ query, operationName: "IntrospectionQuery", operationType: "query" });
}
...
export function printSchemaSummary(result: IntrospectionResult): void {
    ...
    for (const f of schema.queryType.fields) {
        const args = f.args.length > 0 ? `(${f.args.map((a) => `${a.name}: ${formatArgType(a.type)}`).join(", ")})` : "";
    ...
    const key = schema.types.filter((t) => t.kind === "OBJECT" && t.fields && t.name && !t.name.startsWith("__") && /fixture|market|sport|group|template|outcome/i.test(t.name));
    if (key.length > 0) {
        console.log(`\n── Key Types ──\n`);
        for (const t of key) {
            console.log(`  ${t.name} {`);
            for (const f of t.fields || []) console.log(`    ${f.name}: ${f.type.name || f.type.ofType?.name || f.type.kind}`);
            console.log(`  }\n`);
        }
    }
    console.log("═".repeat(55));
