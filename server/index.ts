...
app.get("/api/bets/:id", async (req, res) => {
    const db = getServerDb();
    const rows = await db.select().from(schema.bets).where(eq(schema.bets.id, req.params.id)).limit(1);
...
app.put("/api/presets/:id", async (req, res) => {
    ...
    if (preset.amount !== undefined) updates.amount = preset.amount != null ? String(preset.amount) : null;
    if (preset.percentage !== undefined) updates.percentage = preset.percentage != null ? String(preset.percentage) : null;
    if (preset.unitSize !== undefined) updates.unitSize = preset.unitSize != null ? String(preset.unitSize) : null;
    if (preset.bankroll !== undefined) updates.bankroll = preset.bankroll != null ? String(preset.bankroll) : null;
    if (preset.isDefault !== undefined) updates.isDefault = preset.isDefault;
    await db.update(schema.stakingPresets).set(updates).where(eq(schema.stakingPresets.id, Number(req.params.id)));
    res.json({ ok: true });
});
